...
class BeneficiariesResidenciesTableStats extends BaseWidget
{
    ...
    {
        return $table
            ->heading(__('app.stats.beneficiaries.residencies'))
            ->query(
                fn () => Beneficiary::query()
                    ->whereInShelter(Filament::getTenant())
                    ->join('countries', 'beneficiaries.residence_country_id', 'countries.id')
                    ->select([
